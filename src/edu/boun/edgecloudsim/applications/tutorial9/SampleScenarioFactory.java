...
/**
 * Scenario factory for tutorial9 (identical wiring to tutorial8's), combining the
 * normal-user population from tutorial6 with a separate SAR (Search &amp; Rescue)
 * team population that enters the scenario later and has its own applications
 * and movement policy. Unlike tutorial8, this tutorial's config only sweeps
 * VORONOI policy variants (see {@code uav_mobility_options} in
 * default_config.properties) to compare different SAR priority factors.
...
public class SampleScenarioFactory implements ScenarioFactory {
	...
	public MobilityModel getMobilityModel() {
		...
		// distinct, self-describing result file names. Bare "VORONOI" keeps using
		// the configured sar_priority_factor. To add another factor, just append a
		// new "VORONOI_<factor>" entry to uav_mobility_options - no Java changes needed.
    ...
    /**
     * Creates the UAV mobility model. Unchanged from tutorial8: since both
     * populations share a single device-id space via {@link CombinedMobilityModel},
     * the VORONOI UAV tracking policy swept in tutorial9 (which iterates over
     * every mobile device id) automatically takes SAR members into account as
     * well, once they have entered the scenario.
     */
    @Override
