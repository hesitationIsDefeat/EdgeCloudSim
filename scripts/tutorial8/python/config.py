...
_BW_MARKER_PALETTE = ['-k*', '-ko', '-kv', '--kp', '-k^', '-kd']
_COLOR_MARKER_PALETTE = ['-*', '-o', '-v', '--p', '-^', '-d']

# ONAT: kept for backwards-compatible legend text; scenarios not listed here just
# use their raw name (see _parse_uav_mobility_options).
_LEGEND_OVERRIDES = {'RANDOM': 'RND'}
...
def get_configuration():
    ...
    config = {
        ...
        'output_folder_path': os.path.join('../../../sim_results/tutorial8', _load_tutorial_display_name('tutorial8')),
        'num_iterations': 10,
        'x_tick_interval': 1,
