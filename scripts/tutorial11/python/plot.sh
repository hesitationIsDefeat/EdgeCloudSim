...
# results later doesn't require cross-referencing scripts/tutorial11/config (which may
...
tutorial_name=$(grep "^tutorial11=" "$tutorial_names_file" 2>/dev/null | cut -d'=' -f2- | tr -d '\r')
: "${tutorial_name:=tutorial11}"
results_dir="../../../sim_results/tutorial11/${tutorial_name}"
mkdir -p "$results_dir"
